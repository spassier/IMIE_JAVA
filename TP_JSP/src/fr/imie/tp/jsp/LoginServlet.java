...
public class LoginServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		List<PersonneDTO> userDTOs = (List<PersonneDTO>)request.getSession().getAttribute("userDTOs");
		
		// Recherche du user correspond au login & password
		PersonneDTO userSecured = null;
		
		for (PersonneDTO userDTO : userDTOs)
		{
			if( userDTO.getNom()/*.getLogin()*/.compareTo(login) == 0 && userDTO.getPassword().compareTo(password) == 0 )
			{
				userSecured = userDTO;
				break;
			}
